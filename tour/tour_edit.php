...
<?php
    ...
    echo "<h1 class=\"text-center\">Spectacles</h1>";
            if(isset($_GET["name"]) && isset($_GET["value"]) && $_GET["v"]=="edit")
            {
                ...
                $sql="UPDATE `spectacle` SET `spec_name`='$name',`spec_description`='$desc',`loc_x`='$x',`loc_y`='$y',`popularity`='$pop',`created_by`='$user',`picture`='$pic'  where id_spec='$value'";
            ...
            $city = $_SESSION["city"];
            $sql="select `id_spec`, `spec_name`, `spec_description`, `loc_x`, `loc_y`, `popularity`, `created_by`, `picture` from spectacle where spec_city_id='$city'";
            echo "<tr><td></td><td>id</td>
            <td>name</td><td>description</td><td>X</td><td>Y</td><td>popularity</td><td>creator</td><td>picture</td></tr>";

            echo "<form method=\"get\" action=\"tour_edit.php\">";
        ...
        echo "<div class=\"row\">";
            ...
            if(!empty($_GET["v"]) ){
                if($_GET["v"]=="edit")
                {
                    ...
                    echo "<input type=\"file\" name=\"picture\" placeholder=\"picture\" class=\"button-size col-6  needs-validation\">";
